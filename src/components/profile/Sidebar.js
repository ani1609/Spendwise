...
const Sidebar = ({ onButtonClick }) => {
  ...
  const sidebarClasses = `transition-colors dark:text-white sidebar fixed left-0 top-0 h-screen w-60 text-black transform transition-transform ${
    isSidebarVisible
      ? "translate-x-0 transition-transform duration-300 ease-in"
    ...
    <>
      <div className="z-10">
        <FaBars
          ...
          className="mx-4 my-7 text-3xl absolute cursor-pointer sm:hidden block"
        />
        <div className={sidebarClasses} style={dynamicStyle}>
          <div className="buttons p-4 mt-36  ">
            <FaTimes
              className="absolute top-4 right-4 text-3xl cursor-pointer sm:hidden block"
              onClick={toggleSidebar}
            />
            <button
            ...
            <button
              className="flex items-center mb-8"
              onClick={() => onButtonClick("predefinedTransactions")}
            >
